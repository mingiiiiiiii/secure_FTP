...
// // Function to derive an AES key using HKDF
// unsigned char* deriveAesKey(char* sharedSecret, int sharedSecretLen) {
//     // You may need to adjust these parameters based on your specific requirements
//     const EVP_MD* md = EVP_sha256();
//     const int keyLen = 16; // AES-128 key length in bytes

//     unsigned char* aesKey = (unsigned char*)malloc(keyLen);
//     if (!aesKey) {
//         handleErrors();
//         return NULL;
//     }

//     // HKDF using the shared secret as input keying material
//     if (HKDF(md, sharedSecret, sharedSecretLen, NULL, 0, NULL, 0, aesKey, keyLen) != 1) {
//         handleErrors();
//         free(aesKey);
//         return NULL;
//     }
...
int main()
{
    ...
    // unsigned char* aesKey = deriveAesKey(sharedKey->secret, sharedKey->length);
    unsigned char aesKey1[16] = { 0x00 };
    unsigned char aesKey2[16] = { 0x00 };

    memcpy(aesKey1, secretAlice->secret, 16);
    memcpy(aesKey2, secretBob->secret, 16);

    printf("ALICE\n");
    for (int i = 0; i < 16; i++) {
        printf("%02X ", aesKey1[i]);
    } printf("\n");
